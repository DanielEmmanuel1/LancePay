        ...
        if (!finalIsInternal) {
            if (senderType === 'freelancer') {
                ...
                await sendInvoiceMessageEmail({
                    ...
                    message: content,
                    senderName: finalSenderName,
                }).catch((err) => logger.error('Failed to send message email to client:', err))
            } else {
                // Notify freelancer
                await sendInvoiceMessageEmail({
                    ...
                    message: content,
                    senderName: finalSenderName,
                }).catch((err) => logger.error('Failed to send message email to freelancer:', err))
            }
            ...
            }).catch((err) => logger.error('Failed to dispatch message webhook:', err))
        }
