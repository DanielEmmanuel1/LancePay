...
import { LayoutDashboard, FileText, ArrowUpRight, Settings, LogOut, ChevronLeft, ChevronRight, Menu, X } from 'lucide-react'
  ...
  const sidebarContent = (
    <>
      <div className="p-4 border-b border-brand-border overflow-hidden">
        <Link href="/dashboard" className="flex items-center gap-2 whitespace-nowrap" onClick={() => setMobileOpen(false)}>
          <div className="w-8 h-8 bg-brand-black rounded-lg flex items-center justify-center shrink-0">
            <span className="text-white font-bold text-sm">LP</span>
          </div>
          <span className={cn('font-bold text-xl text-brand-black transition-opacity duration-500', collapsed ? 'opacity-0 lg:opacity-0' : 'opacity-100')}>LancePay</span>
        </Link>
      </div>
